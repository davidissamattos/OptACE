...
class DOO(BlackBox):
    def __init__(self, dimensions_definition, beta=0.5, minimum_grow=1):
        """
       ...
       arms_def is the definition of each arm
        ...
        self.arms_def = dimensions_definition
        if minimum_grow > 0:
        ...
        if beta > 0:
            self.beta = beta  # beta should be greater than 0
        else:
            self.beta = 0.5
            logger.warning("beta should be greater than 0")
    ...
    def get_results(self):
        armvalue = self.get_best_arm_value()
        return armvalue
    ...
    def get_new_trials(self, unit_diversion=[], context=[]):
        ...
        :return:
        """
        # highest_leaves_index = self.get_best_leaves_index()
        # armindex = int(np.random.choice(highest_leaves_index,1)[0])
        #This is better than the other above
        armindex=int(self.get_leaf_to_expand())
        returnarm = self.get_arm_value_for_index(armindex)
        return returnarm
    ...
    def debugPrint(self):
        logger.info('Number of arms: ', self.get_number_of_arms())
        for i in range(self.get_number_of_arms()):
            logger.info('Arm ', i, ': ', self.get_arm_value_for_index(i),
                        ...
                        ' Children (L,R): ', self.get_left_child_index_for_index(i),
                        self.get_right_child_index_for_index(i))
...
## Adding values to the list
    ...
    def update_stats_for_index(self,index,reward):
        ...
        self.arm_list[index,self.M2]= 0
        #self.arm_list[index,self.Mean]=reward
        #self.arm_list[index,self.Counts]= self.arm_list[index,self.Counts]+1
    ...
    #     """
    #     get the highest branch with highest mean value
    #     """
    #
    #     # now that we have only the highest branch
    #     maxheight_vector = np.sum(self.arm_list[:,self.Height:(self.Height+self.number_dimensions)],axis=1)
    #     maxheight_value =int(maxheight_vector.max())
    #     # print 'Sum of the heights: ', maxheight_vector
    #     # print maxheight_value
    #     maxheightchildren = self.arm_list[maxheight_vector[:] == maxheight_value]
    #     #print maxheightchildren
    #     #getting the correct index for the deepest node with the heighest mean
    #     best_arm_index = int(maxheightchildren[np.argmax(maxheightchildren[:, self.Mean]),self.ArmIndex])
    #
    ...
    def get_best_arm_index(self):
        ...
        #get max from the leaves list
        maxvalue = np.max(self.arm_list[:,self.Mean])
        index = np.random.choice(self.arm_list[self.arm_list[:,self.Mean] == maxvalue][:,self.ArmIndex],1)[0]
        return int(index)
    ...
    def calculate_b_for_index(self, index):
        """
        ...
        parent_index = int(self.arm_list[index,self.ParentIndex])
        parent_mean = self.arm_list[parent_index,self.Mean]

        mean = self.arm_list[index,self.Mean]
        h = np.sum(self.get_heights_for_index(index))
        hdelta = 2 **(-h*self.beta)
        if counts > 0:
            b_value = mean + hdelta
        else:
            b_value = np.inf#parent_mean + hdelta
        return b_value
    ...
    def calculate_bound_for_index(self, index):
        ...
        :return:
        """
        b = self.calculate_b_for_index(index)
        self.arm_list[index,self.Bound] = b
    ...
    def calculate_arm_decision_criteria_for_index(self,index):
        ...
        if np.isnan(bound) or np.isinf(bound):
            ...
            self.arm_list[index,self.DecisionCriteria] = bound
    ...
    def get_leaves(self):
        leaves = self.arm_list[np.isnan(self.arm_list[:,self.RightChildren])]
        return leaves


    def get_best_leaves_index(self):
        leaves = self.get_leaves()
        #get max from the leaves list
        maxvalue = np.max(leaves[:,self.Bound])
        #print maxvalue
        indexlist = leaves[leaves[:,self.Bound] == maxvalue][:,self.ArmIndex]
        return indexlist

    def get_leaf_to_expand(self):
