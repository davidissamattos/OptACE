...
from Algorithms.BlackBox.BlackBox import BlackBox

import logging
logger = logging.getLogger(__name__)

class LGHOO(BlackBox):
    def __init__(self, dimensions_definition, v1=1.0, rho=0.5, minimum_grow=1):
        """
...
## Adding values to the list
    ...
    def get_best_arm_index(self):
        ...
        best_arm_index = int(self.arm_list[np.argmax(self.arm_list[:, self.Mean]), self.ArmIndex])
    ...
    def calculate_arm_decision_criteria_for_index(self,index):
        ...
        if np.isnan(bound) or np.isinf(bound):
            ...
            self.arm_list[index,self.DecisionCriteria] = float(self.arm_list[index,self.Mean])/self.calculate_ucb_for_index(index)
